...
export default function LogbookScreen() {
  ...
  const handleViewMCARequirements = async () => {
    ...
    try {
      ...
      console.log('[LogbookScreen iOS] User department:', department, '- navigating to MCA requirements');
      
      setTimeout(() => {
        router.push(`/mca-requirements?department=${department}`);
      }, 100);
    } catch (error) {
      console.error('[LogbookScreen iOS] Failed to get user profile, defaulting to deck pathway:', error);
      setTimeout(() => {
