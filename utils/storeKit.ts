...
Now let me fix the React Hooks warnings in the home screen files:Now I need to fix the React Hooks warning. The issue is that `isLocationStale` is included in the dependency array of `loadActiveVesselLocation`, but it's defined with `useCallback` and has an empty dependency array, so it never changes. The warning is correct - we can remove it from the dependency array.
